                            ...
                            <table class="min-w-full divide-y divide-gray-200 dark:divide-neutral-700">
                                ...
                                <tr>
                                    ...
                                    @foreach (range(1, $daysInMonth) as $day)
                                        <th scope="col"
                                            class="px-6 py-3 text-start border-s
                                            border-gray-200 dark:border-neutral-700">
                                            <span
                                                class="text-xs font-semibold uppercase
                                                @if(Carbon\Carbon::create($year, $month)->day($day)->format('N') == 6)
                                                    text-blue-800
                                                    dark:text-blue-800
                                                @elseif(Carbon\Carbon::create($year, $month)->day($day)->format('N') == 7)
                                                    text-red-800
                                                    dark:text-red-800
                                                @else
                                                    text-gray-800
                                                ...
                                                @endif
                                                ">
                                                {{ $day }} ({{ Carbon\Carbon::create($year, $month)->day($day)->format('D') }})
                                                @if(Carbon\Carbon::create($year, $month)->day($day)->format('N') < 6)
                                ...
                                @if($students->count() > 0)
                                    @foreach ($students as $student)
                                        <tr :key="{{ $student->id }}">
                                            <td class="h-px w-auto whitespace-nowrap border border-r-gray-200">
                                                <div class="px-6 py-2">
                                                <span
                                            ...
                                            @foreach (range(1, $daysInMonth) as $day)
                                                <td class="h-px w-auto whitespace-nowrap">
                                                    <div class="px-6 py-2 text-center items-center justify-center">
                                                    @if(Carbon\Carbon::create($year, $month)->day($day)->format('N') == 6 || Carbon\Carbon::create($year, $month)->day($day)->format('N') == 7)
                                                        -
                                                    @else
                                ...
                                @else
                                    <tr>
                                        <td colspan="{{ $daysInMonth + 1 }}" class="text-center px-2 py-4 items-center justify-center font-medium text-3xl text-red-600">No students are registered.</td>
                                    </tr>
                                @endif
                                </tbody>
                            </table>
